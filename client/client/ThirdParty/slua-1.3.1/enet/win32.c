...
enet_address_set_host (ENetAddress * address, const char * name)
{
    ...
    if (getaddrinfo (name, NULL, & hints, & resultList) != 0) {
      debug_printf("getaddrinfo failed, %s", name);
      return -1;
    }
    ...
    for (result = resultList; result != NULL; result = result -> ai_next)
    {
        if (result -> ai_addr != NULL && result -> ai_addrlen >= sizeof (struct sockaddr_in))
        {
            if (result -> ai_family == AF_INET)
            {
                ...
                ((unsigned int *) & address -> host.s6_addr)[0] = 0;
                ((unsigned int *) & address -> host.s6_addr)[1] = 0;
                ((unsigned int *) & address -> host.s6_addr)[2] = htonl(0xffff);
                ((unsigned int *) & address -> host.s6_addr)[3] = sin->sin_addr.s_addr;
