...
import_field(struct sproto *s, struct field *f, const uint8_t * stream) {
	...
	for (i=0;i<fn;i++) {
		...
		case 4: // array
			if (value)
				array = value << SPROTO_ARRAY_SHIFT;
			break;
		case 5:	// key
			f->key = value;
			break;
...
sproto_dump(struct sproto *s) {
	...
	for (i=0;i<s->type_n;i++) {
		...
		for (j=0;j<t->n;j++) {
			...
			if (f->type & SPROTO_ARRAY_MASK) {
				switch(f->type & SPROTO_ARRAY_MASK)
				{
					case SPROTO_TARRAY:
						array[0] = '*';
						break;
			...
			{
				int t = f->type & ~SPROTO_ARRAY_MASK;
				if (t == SPROTO_TSTRUCT) {
					type_name = f->st->name;
				} else {
					switch(t) {
			...
			if (t == SPROTO_TINTEGER && f->extra > 0) {
				printf("(%d)", f->extra);
			}
			if (f->key >= 0) {
				printf("[%d]", f->key);
...
static inline void
uint32_to_uint64(uint8_t *buffer) {
	buffer[4] = 0;
	buffer[5] = 0;
	buffer[6] = 0;
	buffer[7] = 0;
		...
		if (sz == sizeof(uint32_t)) {
			...
			if (intlen == sizeof(uint32_t)) {
				...
				for (i=index-2;i>=0;i--) {
					...
					uint32_to_uint64(header+1+i*sizeof(uint64_t));
				}
...
// int或者string做key的字典
static int
encode_dict(sproto_callback cb, struct sproto_arg *args, uint8_t *data, int size) {
	int sz;
	int total_sz= 0;
	if (size < SIZEOF_LENGTH)
		return -1;
	...
	return fill_size(data, total_sz);
}
...
sproto_encode(struct sproto_type *st, void * buffer, int size, sproto_callback cb, void *ud) {
	...
	for (i=0;i<st->n;i++) {
		...
		args.extra = f->extra;
		args.type = type & ~SPROTO_ARRAY_MASK;
		args.array_type = type & SPROTO_ARRAY_MASK;
		if (args.array_type) {
			switch(args.array_type)
			{
				case SPROTO_TARRAY:
					...
					break;
				case SPROTO_TIDICT:
					sz = encode_dict(cb, &args, data, size);
					break;
				case SPROTO_TSDICT:
					sz = encode_dict(cb, &args, data, size);
					break;
...
static int 
decode_dict(sproto_callback cb, struct sproto_arg *args, uint8_t * stream) {
	uint32_t sz = todword(stream);
	stream += SIZEOF_LENGTH;

	args->value = stream;
	args->length = sz;
	// do once to create empty table
	do {
		if(cb(args)) return -1;
	} while(args->length > 0);
	return 0;
...
sproto_decode(struct sproto_type *st, const void * data, int size, sproto_callback cb, void *ud) {
	...
	for (i=0;i<fn;i++) {
		...
		args.tagname = f->name;
		args.tagid = f->tag;
		args.type = f->type & ~SPROTO_ARRAY_MASK;
		args.subtype = f->st;
		args.index = 0;
		args.mainindex = f->key;
		args.array_type = f->type & SPROTO_ARRAY_MASK;
		args.extra = f->extra;
		if (value < 0) {
			if (args.array_type) {
				switch(args.array_type)
				{
					...
					case SPROTO_TIDICT:
					case SPROTO_TSDICT:
						if (decode_dict(cb, &args, currentdata)) {
							return -1;
						}
						break;
				}
		...
		} else if (f->type != SPROTO_TINTEGER && f->type != SPROTO_TBOOLEAN && f->type != SPROTO_TFLOAT) {
			return -1;
		} else {
			uint64_t v = value;
			args.value = &v;
