...
Vector3 = 
{	
	__typename = "Vector3",  -- 注意__typename是C#中识别类型的凭证
}
...
function Vector3.NewByVector2(v2)
	return Vector3.New(v2.x, 0, v2.z)
end

function Vector3.NewByVector3(v3)
...
function Vector3.Distance(va, vb)
	local za = va.z or 0
	local zb = vb.z or 0
	return sqrt((va.x - vb.x)^2 + (va.y - vb.y)^2 + (za - zb)^2)
end

function Vector3.SqrDistance(va, vb)
	local za = va.z or 0
	local zb = vb.z or 0
	return (va.x - vb.x)^2 + (va.y - vb.y)^2 + (za - zb)^2
end
...
function Vector3.SqrDistance2D(va, vb)
...
function Vector3:DistanceToSegment(p1, p2)
  local ab = p2 - p1
  local ac = self - p1
  local f = dot(ab, ac)
  if f < 0 then return Vector3.Distance(p1, self), p1 end
  local d = dot(ab, ab)
  if f > d then return Vector3.Distance(p2, self), p2 end
  f = f / d;
  local p = p1 + ab * f
  return Vector3.Distance(self, p), p
end
...
Vector3.__mul = function(va, d)
	...
	elseif type(va) == "number" then
		return Vector3.New(d.x * va, d.y * va, d.z * va)
	else
		local vec = va:Clone()
		vec:MulQuat(d)
		return vec
